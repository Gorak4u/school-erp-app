...
export default function ForgotPasswordPage() {
  ...
  const [isEmailServiceError, setIsEmailServiceError] = useState(false);
    ...
    try {
      ...
      if (!res.ok) {
        if (data.error === 'EMAIL_SEND_FAILED') {
          setError(data.message || 'There is a problem with the email service. Please contact your school administrator to reset your password manually.');
          setIsEmailServiceError(true);
        } else {
          setError(data.error || 'An error occurred. Please try again.');
          setIsEmailServiceError(false);
        }
      } else {
        setIsSubmitted(true);
        setIsEmailServiceError(false);
                        ...
                        <AnimatePresence>
                          {error && (
                            <motion.div
                              ...
                              animate={{ opacity: 1, y: 0 }}
                              exit={{ opacity: 0, y: -10 }}
                              className={`${isEmailServiceError 
                                ? 'bg-orange-500/20 border border-orange-500/50 text-orange-400' 
                                : 'bg-red-500/20 border border-red-500/50 text-red-400'} rounded-lg p-4 text-sm`}
                            >
                              {isEmailServiceError && (
                                <div className="flex items-center gap-2 mb-2">
                                  ...
                                  <span className="font-semibold">Email Service Issue</span>
                                </div>
                              )}
                              <div>{error}</div>
                              {isEmailServiceError && (
                                <div className="mt-3 pt-3 border-t border-orange-500/30 text-xs">
                                  <p className="mb-1">Please contact your school administrator with:</p>
                                  <ul className="list-disc list-inside space-y-1 text-orange-300">
                                    <li>Your email address: {email}</li>
                                    <li>The time you tried to reset</li>
                                    <li>Request manual password reset</li>
                                  </ul>
                                </div>
                              )}
                            </motion.div>
